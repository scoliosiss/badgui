...
class drawbut {
  ...
  togglebutton(min, max) {
    ...
    if (!file.includes(`/*${newtext}righttrue`) && !file.includes(`/*${newtext}rightfalse`) && this.type < 4) FileLib.write(configpath, configpath2, `${file}/*${newtext}rightfalse*/ function ${newtext}righttog() {let configfile = FileLib.read("${configpath}", "${configpath2}"); let fortnite = false; if (configfile.includes("/*${newtext}right"+"true*/")) fortnite = true; else fortnite = false; return fortnite}; export{${newtext}righttog}\n`);
    if (!file.includes(`/*${newtext}true`) && !file.includes(`/*${newtext}false`) && this.type < 4) FileLib.write(configpath, configpath2, `${file}/*${newtext}false*/ function ${newtext}tog() {let configfile = FileLib.read("${configpath}", "${configpath2}"); let fortnite = false; if (configfile.includes("/*${newtext}"+"true*/")) fortnite = true; else fortnite = false; return fortnite}; export{${newtext}tog}; /*${newtext}rightfalse*/ function ${newtext}righttog() {let configfile = FileLib.read("${configpath}", "${configpath2}"); let fortnite = false; if (configfile.includes("/*${newtext}right"+"true*/")) fortnite = true; else fortnite = false; return fortnite}; export{${newtext}righttog}\n`);
